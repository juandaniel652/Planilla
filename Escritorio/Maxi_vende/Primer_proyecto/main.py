...
from kivymd.uix.list import OneLineAvatarIconListItem, MDList, OneLineListItem, IconRightWidget
...
from actualizaciones.actualizador import Actualizador
import database
...
class MainApp(Box):
            ...
            self.dialog = MDDialog(
                ...
                buttons=[
                    MDRaisedButton(text="Cerrar", on_release=lambda x: self.dialog.dismiss(), 
                                font_name = "Ancizar_botones", font_size = "18sp", 
                                text_color = '#FFFFFF', md_bg_color  ='#00C853')
                ],
                size_hint=(0.9, None),
    ...
    def mostrar_productos_por_fecha (self, instance, titulo_fecha) : 
        ...
        contenedor = BoxLayout(orientation='vertical', spacing=15, padding=10)

        # ScrollView y contenido
        scroll = MDScrollView(size_hint=(1, 1))

        # GridLayout configurado para scroll
        ...
        lineas_separadoras_y_totales = [
            "_______", "_______", "_______", "_______",
            "", "", "", "",
            "Total Vendido", f"${total_precio[0][0]}", 
            "", "", "", "", "", "", 
            "Total Cantidad", f"{total_cantidad[0][0]}"
        ]
        ...
        for item in datos:
            for valor in item:
                self.grid.add_widget(MDLabel(
                    text=str(valor) if not isinstance(valor, float) else f"${valor}",
                    font_name="Ancizar_contenido",
                    halign="left",
                    size_hint_y=None,
                    height="30dp"
        ...
        # Agregar líneas y totales
        for columnas in lineas_separadoras_y_totales:
            self.grid.add_widget(MDLabel(
                text=columnas,
                font_name="Ancizar_contenido",
                halign="left",
...
class MainScreen(BoxLayout):
    def __init__(self, **kwargs):
        ...
        self.boton = Button(text="Actualizar", 
                            disabled=True, 
                            font_name = "Ancizar_botones",
                            height = 30,
                            background_color = '#00C853',
                            color = '#FFFFFF',  
                            on_press=self.iniciar_descarga)
        ...
        # Verificar actualización solo al inicio
        Clock.schedule_once(lambda dt: self.verificar(), 1)

    def verificar(self):
        info = self.actualizador.verificar_actualizacion()
        if info:
            self.label.text = f"Nueva versión {info['version']} disponible"
            self.boton.disabled = False
        else:
            # Si no hay actualización, cerrar esta pantalla o cambiarla
            App.get_running_app().stop()  # o cambia la vista principal


    def iniciar_descarga(self, instance):
        self.label.text = "Descargando..."
        self.actualizador.descargar_apk(
            progreso_callback=self.actualizar_progreso,
            completado_callback=self.iniciar_instalacion
        )

    def actualizar_progreso(self, progreso):
        self.progress.value = progreso
        self.label.text = f"Descargando... {progreso}%"

    def iniciar_instalacion(self, *_):
        self.label.text = "Instalando actualización..."
        self.actualizador.instalar_apk()
...
class NegocioApp(MDApp):
    ...
    def build(self):
        ...
        #info = actualizador.verificar_actualizacion()
