...
def obtener_ultimas_fechas():
    """
    Devuelve lista de tuplas con:
    (numero_territorio, fecha_completado_última)
    Solo incluye territorios 21 a 40.
    """
    conexion = None
    cursor = None
    ...
    try:
        ...
        WHERE t.numero BETWEEN 21 AND 40
        GROUP BY t.numero
...
def mostrar_territorios_mas_atrasados(top=11):
    ...
    # Ordenar por fecha más antigua primero (o nunca hecho)
    datos_ordenados = sorted(
        datos,
        key=lambda x: x[1] if x[1] is not None else datetime.min
    )

    print("\n📌 **TOP Territorios más atrasados (21–40)**")
    print("-" * 55)
